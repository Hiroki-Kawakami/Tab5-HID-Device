...
static void display_mux_gui_setup(void) {
    ...
    lv_indev_set_read_cb(indev, display_mux_gui_input_read);
}

// MARK: Layout
#define LAYOUT_BUFFER_SIZE (720 * 1280 * 2)
static void *layout_base_image, *layout_active_image;
...
void display_mux_layout_load_images(const layout_image_t *base, const layout_image_t *active) {
    layout_base_image = display_mux_layout_load_image(base, layout_base_image);
    layout_active_image = display_mux_layout_load_image(base, layout_active_image);
}

static void display_mux_layout_draw_base_image(void) {
    esp_err_t err = ppa_do_scale_rotate_mirror(gui_ppa, &(ppa_srm_oper_config_t){
        .in = {
            .buffer = layout_base_image,
            .pic_w = 720,
            .pic_h = 1280,
            .block_w = 720,
            .block_h = 1280,
